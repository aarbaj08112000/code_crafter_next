...
export default function Navbar() {
  ...
  return (
    <header className="w-full fixed top-0 left-0 z-50 transition-all duration-300">
      <nav className="mx-auto max-w-7xl mt-4 px-6 relative">
        <div className={`glass-nav shadow-lg rounded-2xl bg-white flex flex-col border border-white/20 transition-all duration-300 ${isMobileMenuOpen ? "py-4 px-6" : "px-6 py-3"}`}>
          ...
          {isMobileMenuOpen && (
            <div className="mt-6 flex flex-col md:hidden animate-in fade-in slide-in-from-top-2 duration-300 w-full">
                ...
                {navLinks.map((link) => (
                  <li key={link.path}>
                    <Link
                      ...
                      className={`block text-sm font-semibold transition-colors duration-300 ${pathname === link.path
                        ? "text-[#8a4785]"
                        : "text-gray-600 hover:text-[#8a4785]"
                        }`}
                    >
